...
const app = createApp({
    ...
    methods: {
        ...
        /** 從 tap_des 取出「說明文字」：若為 JSON 只取 comment，絕不顯示整段 JSON */
        extractCommentFromTapDes(tapDes) {
            if (tapDes == null) return '';
            const s = String(tapDes).trim();
            if (s === '') return '';
            if (s.charAt(0) === '{') {
                try {
                    const j = JSON.parse(s);
                    if (j && typeof j === 'object' && Object.prototype.hasOwnProperty.call(j, 'comment')) return String(j.comment || '');
                    return '';
                } catch (e) { return ''; }
            }
            return s;
        ...
        async checkExistingApplication() {
            try {
                ...
                if (data.ok && data.application) {
                    ...
                    // 備註說明：一律只顯示「說明文字」，絕不以 JSON 呈現
                    this.form.comment = this.extractCommentFromTapDes(app.tap_des);
                    // 類組：優先 tap_group / group（API 已用 tap_group 關聯 groupdata），僅舊資料才從 tap_des 解析
                    let rawGroup = app.tap_group ?? app.group?.group_ID;
                    if (!rawGroup && app.tap_des && String(app.tap_des).trim().startsWith('{')) {
                        try {
                            const j = JSON.parse(app.tap_des);
                            if (j && j.group_id != null && j.group_id !== '') rawGroup = String(j.group_id);
                        } catch (e) {}
                    }
                    if (rawGroup) this.form.group_id = String(rawGroup);
