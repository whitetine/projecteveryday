            ...
            <div class="modal-content">
                ...
                <div class="modal-body req-detail-card">
                    ...
                    <div class="req-detail-section">
                        <label class="req-detail-label">回報說明：<span v-if="now_requirement.status==0">小組尚未回報</span></label>
                        <p class="req-detail-text" v-if="now_requirement.status!=0">{{ return_form.rp_comment }}</p>
                    </div>
                    <div class="req-detail-section" v-if="now_requirement.status!=0">
                        ...
                        <div class="req-detail-tags req-count-row">
                            <div v-for="(j,key) in now_requirement.req_count" class="req-count-item">
                                ...
                                <input type="number" class="form-control" v-model="return_form.count2" v-if="key==1 && req_return_edit" :readonly="now_requirement.status!=0">
                            </div>
                        </div>
                    </div>
                    <div class="req-detail-section" v-if="now_requirement.status!=0">
                        ...
                        <label class="req-detail-label" v-if="now_requirement.status!=0">{{now_requirement.status==3?'通過':'最後審核'}}時間：{{now_rp.rp_approved_d}}</label>
                    </div>
                    <div class="req-detail-section" v-if="now_requirement.status==1">
                        <label class="req-detail-label">指導老師建議：</label>
                        <textarea class="form-control" v-model="return_form.rp_remark" rows=4></textarea>
                    </div>
                    <div class="req-detail-section" v-if="now_rp?.rp_remark && now_requirement.status!=1">
                        <label class="req-detail-label">指導老師建議：</label>
                        <p class="req-detail-text">{{ now_rp.rp_remark }}</p>
                    </div>
                </div>
                <div class="modal-footer" v-if="now_requirement.status==1">
                    <button class="btn btn-danger" @click="req_return_NotOKclick" style="margin-right:14px">小組成員請修正</button>
                    <button class="btn btn-primary" @click="req_return_OKclick">指導老師已閱，並通過</button>
                </div>
            </div>
        </div>
    </div>
